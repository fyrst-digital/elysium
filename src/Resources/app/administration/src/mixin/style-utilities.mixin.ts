    ...
    Component.wrapComponentConfig({
        ...
        methods: {
                ...
                Object.entries(breakpointListeners).forEach(
                    ([view, mediaQuery]) => {
                        ...
                        matchMedia.addEventListener(
                            'change',
                            (event) =>
                                event.matches && (this.currentView = view)
                        );
                    }
                );
            },
            ...
            viewStyle(responsiveStyles: object) {
                ...
                const currentBreaktpointIndex = Object.entries(
                    mergedStyles
                ).findIndex((element) => element[0] === this.currentView);

                const accumulatedStyles = Object.values(mergedStyles).reduce(
                    (acc, value, index) => {
